...
public:
  ...
  Matrix2D(size_t n_rows, size_t n_cols)
      : n_0(n_rows), n_1(n_cols), stride_0(n_1), stride_1(1), data({}) {
    // Initialize the empty data vector with the default "Type" value
    data.resize(n_rows * n_cols, Type());
    shape = {n_0, n_1};
    strides = {stride_0, stride_1};
  }
};

// TODO deprecate vectors, move matrices to extensible OOP approach
namespace la {
// Helper structures
...
vec subvector(const vec &v, int start,
              int end); // End exclusive - as with iterators
...
std::pair<double, matrix> gauss(const matrix &LHS,
                                matrix &&RHS); // perform the gauss algorithm,
                                               // getting det. and inv. together
std::pair<double, matrix> gauss(const matrix &LHS, const vec &RHS);

// --- Sherman-Morrison formula

/// @brief Apply the Sherman-Morrison formula to compute the inverse of a matrix
/// A modified by a rank-1 update uv^T, i.e. we are looking for (A + u*v^t)^-1 =
...
/// @param v_T_A_inv The product of the transpose of vector v and the inverse of
/// A
/// @param denominator The denominator of the Sherman-Morrison formula
/// @return The inverse of the modified matrix
matrix ShermanMorrisonFull(const matrix &A_inv, const vec &A_inv_u,
                           const vec &v_T_A_inv, const double denominator);

/// @brief Apply the Sherman-Morrison formula to compute the inverse of a matrix
/// A modified by a rank-1 update uv^T. This is the standard formula, i.e. we
/// are looking for (A + u*v^t)^-1 = A^-1 - (A^-1 * u * v^T * A^-1) / (1 + v^T *
/// A^-1 * u)
...
/// @brief Perform the Sherman-Morrison formula for a row vector v, that is
/// added to each row of A, wich corresponds to the situation where u is the one
/// vector
matrix ShermanMorrisonRow(const matrix &A, const vec &v);

/// @brief Perform the Sherman-Morrison formula for a column vector u, that is
/// added to each column of A, wich corresponds to the situation where v is the
/// one vector
