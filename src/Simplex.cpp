...
util::vec Simplex::distillSolution(const util::vec& x_B, const std::vector<int>& B_set, const int& n) {
    util::vec sol(n, 0.0);
...
std::pair<util::vec, Simplex::Status> Simplex::solve(util::matrix&& lhs, util::vec&& rhs, util::vec&& obj, util::vec&& init) {
    ...
    N_set.reserve(n-m);
    B_set.reserve(m); // The matrix corresponding to the basic set should be invertible - unique identifier of the corresponding edge
