...
void Simplex::Variable::addConstraint(la::matrix lhs, la::vec rhs, ConstrType type)
{
    ...
    default:
        throw std::invalid_argument("Undefined constraint type!");
    }
}
        ...
        if (i_N == -1)
        {
            ...
            if (objective_history.size() > 1)
            {
                ...
                for (size_t i = 0; i < std::min(objective_history.size(), size_t(5)); i++)
                {
                    ...
                    if (i < std::min(objective_history.size(), size_t(5)) - 1)
                        std::cout << " -> ";
                }
                if (objective_history.size() > 5)
                    std::cout << " -> ... -> " << current_objective;
                std::cout << std::endl;
            }
